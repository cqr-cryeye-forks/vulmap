...
def proxy_set(pr, pr_mode):
    ...
    try:
        proxy_ip = str(re.search(r"(.*):", pr)[1])
        proxy_port = int(re.search(r":(.*)", pr)[1])
    ...
    try:
        ...
        proxy_ip_info_dict = "[region: ???] [city: ???] [proxy ip: ???]"
    print(now.timed(de=0) + color.yel_info() + color.yellow(f" Use custom proxy: {pr}"))
    print(now.timed(de=0) + color.yel_info() + color.yellow(f" Proxy info: {proxy_ip_info_dict}"))
