...
def require():
    ...
    def install_gevent():
        try:
            ...
            pwd_gevent = f"{pwd_packages}/gevent"
            os.chdir(pwd_gevent)
            try:
                if os.system("python3 setup.py install >> /dev/null 2>&1") == 0:
                    print(now.timed(de=0) + color.yel_info() + color.yellow(f" gevent install to: {pwd_packages}"))
    ...
    def install_crypto():
        try:
            ...
            t.extractall(path=pwd_packages)
            pwd_crypto = f"{pwd_packages}/pycryptodome"
            os.chdir(pwd_crypto)
            try:
                if os.system("python3 setup.py install >> /dev/null 2>&1") == 0:
                    print(
                        now.timed(de=0) + color.yel_info() + color.yellow(f" pycryptodome install to: {pwd_packages}"))
                    print(now.timed(de=0) + color.yel_info() + color.yellow(
